...
function () {
  ...
  _createClass(Difference, [{
    key: "bindTriggers",
    value: function bindTriggers(column, columnItems, counter) {
      column.querySelector('.plus').addEventListener('click', function () {
    ...
    value: function hideItems(items) {
      items.forEach(function (item, i, arr) {
        if (i !== arr.length - 1) {
          // скрываем все элементы кроме последнего .
  ...
  _createClass(Form, [{
    ...
    value: function initMask() {
      var setCursorPosition = function setCursorPosition(pos, elem) {
        // функция для красивой установки курсора .
        elem.focus();
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          var api;
          item.closest('.join__evolution') || item.classList.contains('form1') ? api = _this.path.joinEvol : api = _this.path.schedule;
          console.log(api);

          _this.postData(api, jsonFile).then(function (res) {
            console.log(res);
  ...
  _createClass(VideoPlayer, [{
        ...
        btn.addEventListener('click', function () {
          if (!btn.closest('.module__video-item') || btn.closest('.module__video-item').getAttribute('data-disabled') !== 'true') {
            ...
            if (document.querySelector('iframe#frame')) {
              // условие чтобы каждый раз не создавать экземпляр класса
              _this.overlay.style.display = 'flex';

              if (_this.path !== btn.getAttribute('data-url')) {
                // если плеер ранее запускался, и уже имеет ссылку, переписываем на новую ссылку из дата атрибута .
                _this.path = btn.getAttribute('data-url');

                _this.player.loadVideoById({
                  videoId: _this.path
                }); // (из YouTube iframe player API) функция служит для загрузки и воспроизведения указанного видео /

              }
            } else {
              // иначе если ранее плеер не запускался ,
              _this.path = btn.getAttribute('data-url'); // берем url c data-атрибута кнопки открытия плеера
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // полностью копируем кнопку .

        if (state.data === 0) {
          // status=0 значит, если видео досмотрено (из документации YT API)
          if (blockedElem.querySelector('.play__circle').classList.contains('closed')) {
            // класс затемняет кнопку.
    ...
    value: function init() {
      if (this.btns.length > 0) {
        // проверяем передавались ли кнопки открытия плеера
        var tag = document.createElement('script'); // из YouTube iframe player API
...
function (_Slider) {
  ...
  _createClass(MainSlider, [{
    ...
    value: function render() {
      if (this.container) {
        // если такой элемент на странице существует .
        try {
      ...
      this.prev.addEventListener('click', function () {
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== 'BUTTON') {
            ...
            _this2.container.insertBefore(active, _this2.slides[0]); //  последний слайд премещаем на первую позицию .
    ...
    value: function init() {
      ...
      try {
        ...
        this.next.parentNode.addEventListener('mouseenter', function () {
          clearInterval(_this3.autoSlide);
        });
        this.next.parentNode.addEventListener('mouseleave', function () {
          if (_this3.autoplay) {
            _this3.autoSlide = setInterval(function () {
              return _this3.nextSlide();
            }, 5000);
          }
        });
...
var Slider = function Slider() {
  ...
  // default  - значения во избежания ошибок, если какой то элемент забыли передать .
